...
I go crazy when I hear a cymbal"""
  ...
  test("Challenge 6 Determine average Hamming distance for various key sizes") {
    val tryKeySize = for (source <- managed(scala.io.Source.fromURL(getClass.getResource("/6.txt")))) yield {
    ...
    /*val input = HexString("1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736")
            val results = for { keysize <- 1 to 5 } yield (keysize, averageHammingDistance(input.toBase64String, keysize, 3))
            val (keySize, minHammingDistance) = results.minBy(_._2)
            println(s"result is keysize $keySize with Hamming distance $minHammingDistance")
            val hexString = input*/
    ...
    val decodedString = hexString.xorWithRepeatingKey(HexString.fromAsciiString(passphrase)).toAsciiString
